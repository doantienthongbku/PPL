...
class ASTGeneration(MT22Visitor):
    ...
    def visitForstmt(self, ctx:MT22Parser.ForstmtContext):
        init = AssignStmt(Id(ctx.scalar_var().getText()), self.visit(ctx.expr(0)))
    ...
    def visitExpr2(self, ctx:MT22Parser.Expr2Context):
        if ctx.expr2():
            op = self.AND().getText() if self.AND() else self.OR().getText()
    ...
    def visitExpr3(self, ctx:MT22Parser.Expr3Context):
        if ctx.expr3():
            op = self.ADD().getText() if self.ADD() else self.SUB().getText()
    ...
    def visitArraylit(self, ctx:MT22Parser.ArraylitContext):
        return self.visit(ctx.subarrayitem())
    ...
    def visitIntlist(self, ctx:MT22Parser.IntlistContext):
        if ctx.getChildCount() == 1:
            ...
            return [IntegerLit(int(ctx.INTLIT().getText()))] + self.visit(ctx.intlist())
