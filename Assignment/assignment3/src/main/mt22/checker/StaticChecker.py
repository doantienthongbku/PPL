...
class Function_Decl:
    def __init__(self, name, param, ret, inherit=None, is_inherit=False, is_spec=False):
        self.name = name
        self.param = param
        self.ret = ret
...
class StaticChecker(Visitor, Utils):
    def __init__(self, ast):
        ...
        self.is_prototype = False   # check if in prototype
        self.in_loop = False    # check if in loop
        self.current_function = None    # current function
        self.has_super = False  # check if has super
    
    def check(self):
        global_env = [[
            Function_Decl(name="readInteger", param=[], ret=IntegerType(), is_spec=True),
            Function_Decl(name="readFloat", param=[], ret=FloatType(), is_spec=True),
            Function_Decl(name="readBoolean", param=[], ret=BooleanType(), is_spec=True),
            Function_Decl(name="readString", param=[], ret=StringType(), is_spec=True),
            Function_Decl(name="printInteger", param=[], ret=VoidType(), is_spec=True),
            Function_Decl(name="printFloat", param=[], ret=VoidType(), is_spec=True),
            Function_Decl(name="printBoolean", param=[], ret=VoidType(), is_spec=True),
            Function_Decl(name="printString", param=[], ret=VoidType(), is_spec=True),
            Function_Decl(name="super", param=[], ret=None, is_spec=True),
            Function_Decl(name="preventDefault", param=[], ret=None, is_spec=True),
        ]]
    ...
    def visitParamDecl(self, ast: ParamDecl, env):
        is_redeclared = self.lookup(ast.name, env, lambda x: x.name)
        if is_redeclared is not None and not is_redeclared.is_inherit:
        ...
        env += [Param_Decl(name, typ, is_inherit=is_inherit, is_out=is_out)]
    ...
    def visitFuncDecl(self, ast: FuncDecl, env):
        ...
        if not self.is_prototype:
            # check inheherit function
            if ast.inherit is not None:
                for sub_env in env:
                    is_inherit_func = self.lookup(ast.inherit, sub_env, lambda x: x.name)
                    if is_inherit_func is not None: break
                
                if is_inherit_func is None:
                    raise Undeclared(Function(), ast.inherit)
                if not isinstance(is_inherit_func, Function_Decl):
                    raise Undeclared(Function(), ast.inherit)
                if is_inherit_func.is_spec:
                    raise Undeclared(Function(), ast.inherit)
                
                if len(is_inherit_func.param) != 0: self.has_super = True
            ...
            self.prototypes.append(Function_Decl(name=ast.name, param=param_list, ret=ast.return_type))
    ...
    def visitBlockStmt(self, ast:BlockStmt, env):
        if len(ast.body) != 0:
            ...
            if self.has_super:
                if first_stmt.name != "super" and first_stmt.name != "preventDefault":
                    raise InvalidStatementInFunction(self.current_function)
        for x in ast.body:
            self.visit(x, env)
    ...
    def visitCallStmt(self, ast, env):
        ...
        if len(ast.args) != len(func.param) and not func.is_spec:
            raise TypeMismatchInStatement(ast)
